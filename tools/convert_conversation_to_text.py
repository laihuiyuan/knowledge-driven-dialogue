...
File: convert_conversation_to_text.py
"""
...
def preprocessing_conversation(text, topic_generalization=False):
    """
    ...
    topic_dict = {}
    if u"电影" == domain_a:
        topic_dict["v_a"] = topic_a
    else:
        topic_dict["p_a"] = topic_a

    if u"电影" == domain_b:
        topic_dict["v_b"] = topic_b
    else:
        topic_dict["p_b"] = topic_b
...
def convert_conversation_to_text(corpus_file, text_file, topic_file,
    ...
    with open(corpus_file, 'r') as f:
        for i, line in enumerate(f):
            model_text, topic_dict = preprocessing_conversation(line.strip(),
...
def main():
    ...
    main
    """
    convert_conversation_to_text(sys.argv[1],
                                 sys.argv[2],
                                 sys.argv[3],
                                 int(sys.argv[4]) > 0)
