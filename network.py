...
from source.elmo.elmo_embed import elmo_embed
...
def model_config():
    ...
    data_arg.add_argument("--char_file", type=str, default="./data/char.dic")
    data_arg.add_argument("--embed_file", type=str, default="./data/cn.vector.txt")

    # Network
    net_arg = parser.add_argument_group("Network")
    net_arg.add_argument('--char_size', type=int, default=50)
    net_arg.add_argument("--embed_size", type=int, default=300)
    net_arg.add_argument("--hidden_size", type=int, default=600)
    net_arg.add_argument('--projection_dim', type=int, default=50)
    ...
    net_arg.add_argument('--max_chars', type=int, default=5)
    ...
    net_arg.add_argument("--filters", type=list, default=[[1, 64], [2, 128], [3, 128], [4, 32]])
    ...
    train_arg.add_argument("--num_epochs", type=int, default=12)
    ...
    # misc_arg.add_argument("--ckpt", type=str, default="models/best")
...
def main():
    ...
    corpus = KnowledgeCorpus(data_dir=config.data_dir, data_prefix=config.data_prefix,
                             min_freq=0, max_vocab_size=config.max_vocab_size,max_chars=config.max_chars,
                             char_size=config.char_size,char_file=config.char_file,
    ...
    # Model definition
    model = KnowledgeSeq2Seq(corpus=corpus, src_vocab_size=corpus.SRC.vocab_size,
                             tgt_vocab_size=corpus.TGT.vocab_size, embed_size=config.embed_size,
                             char_size=config.char_size,hidden_size=config.hidden_size,
                             ...
                             concat=config.decode_concat,char_file=config.char_file,max_chars=config.max_chars,
                             filters=config.filters,activation="relu",proj_dim=config.projection_dim, use_cuda=config.use_gpu)
