...
def calc_distinct_ngram(pair_list, ngram, ind=0):
    """
    calc_distinct_ngram
    """
    ngram_total = 0.0
    ...
    sen_dict = {}
    for pair in pair_list:
        get_dict(pair[ind], ngram, sen_dict)
    for key, freq in sen_dict.items():
        ngram_total += freq
        ngram_distinct_count += 1
        # if freq == 1:
        #    ngram_distinct_count += freq
    try:
        dist=ngram_distinct_count / ngram_total
    except:
        dist=0
    return dist


def calc_distinct(pair_list, ind=0):
    """
    calc_distinct
    """
    distinct1 = calc_distinct_ngram(pair_list, 1, ind)
    distinct2 = calc_distinct_ngram(pair_list, 2, ind)
